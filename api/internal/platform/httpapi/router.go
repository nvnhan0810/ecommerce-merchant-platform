...
func NewRouter(deps Dependencies) http.Handler {
	...
	r.Route("/api/v1", func(api chi.Router) {
		api.With(OptionalBearerAuth(deps.Tokens)).Get("/products", deps.Catalog.ListProducts)
		api.With(OptionalBearerAuth(deps.Tokens)).Get("/products/{id}", deps.Catalog.GetProduct)
		api.With(OptionalBearerAuth(deps.Tokens)).Get("/categories", deps.Catalog.ListCategories)
		api.Get("/media/*", deps.Catalog.ServeMedia)
		api.With(OptionalBearerAuth(deps.Tokens)).Get("/merchants/{id}", deps.Identity.GetMerchant)
		...
		api.Group(func(merchant chi.Router) {
			...
			merchant.Put("/auth/merchant/me", deps.Identity.UpdateMerchantProfile)
			merchant.Post("/auth/merchant/me/avatar", deps.Identity.UploadMyMerchantAvatar)
			merchant.Delete("/auth/merchant/me/avatar", deps.Identity.DeleteMyMerchantAvatar)
			merchant.Get("/merchant/categories", deps.Catalog.ListMerchantCategories)
			merchant.Post("/merchant/categories", deps.Catalog.CreateMerchantCategory)
			...
			merchant.Delete("/merchant/products/{id}/categories/{categoryId}", deps.Catalog.RemoveMerchantProductCategory)
		...
		api.Group(func(admin chi.Router) {
			...
			admin.Post("/merchants/{id}/avatar", deps.Identity.UploadMerchantAvatar)
			admin.Delete("/merchants/{id}/avatar", deps.Identity.DeleteMerchantAvatar)
			...
			admin.Post("/categories", deps.Catalog.CreateAdminCategory)
			admin.Get("/categories/{id}", deps.Catalog.GetCategory)
			admin.Put("/categories/{id}", deps.Catalog.UpdateCategory)
			admin.Patch("/categories/{id}/status", deps.Catalog.UpdateCategoryStatus)
			admin.Delete("/categories/{id}", deps.Catalog.DeleteCategory)
			...
			admin.Delete("/products/{id}/categories/{categoryId}", deps.Catalog.RemoveProductCategory)
